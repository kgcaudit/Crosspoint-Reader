...
### 5.4 릴리스 서명 키 만들기 (한 번만)

휴대폰에 까는 APK 는 이 PC 에만 있는 비공개 키로 서명한다. 저장소(공개)에 들어 있는 개발용 키로 서명하면
휴대폰이 "잠재적으로 유해한 앱" 으로 표시한다 — 누구나 그 키로 서명할 수 있기 때문이다.
...
3. (클라우드 세션이 APK 를 만들게 하려면) 클라우드 환경 설정 → 환경 변수에 두 개를 넣는다:
   ...
   `OLO_RELEASE_PASSWORD` = `.olo\olo-release.password.txt` 의 내용. 새 세션부터 적용된다.
   채팅창에는 붙여 넣지 않는다.

키를 처음 바꾼 뒤에는 휴대폰의 옛 OLO eBook 을 지우고 새 APK 를 깔아야 한다(서명이 다르면 덮어 깔리지
않는다). 그다음부터는 계속 덮어 깔린다. APK 파일 이름에 `-devkey` 가 붙어 있으면 비공개 키를 못 찾은
것이다 — 그 파일은 깔지 않는다.
