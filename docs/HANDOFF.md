...
마지막 갱신: 2026-09-25 · 브랜치 `claude/android-epub-viewer-text-measurer-oqy6rq`
...
```bash
cd android && ./gradlew check                 # 866개 + lint
./gradlew :app:assembleRelease                # → app/build/outputs/apk/release/OLO-eBook-0.17.0-devkey-release.apk  (비공개 키가 있으면 -devkey 없음)
...
**다음: PDF 찾기 · 형광펜 · 듣기** — 엔진은 플랫폼 `PdfRenderer` 그대로(결정 P1 재확인, 아래 §5), 안드로이드 15+ 의
글자 API(`getTextContents` · `searchText` · `selectContent` · `getLinkContents`)를 쓴다. 기능마다 **구상안부터**.
