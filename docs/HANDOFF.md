...
./gradlew :app:assembleRelease                # → app/build/outputs/apk/release/OLO-eBook-0.15.1-release.apk  (비공개 키가 없으면 -devkey)
...
- **서명(0.15.1 에서 바꿈)**: release APK 는 **저장소 밖의 비공개 키**로 서명한다. 0.15.0 까지는 커밋된
  `app/olo-dev.keystore` 로 서명했는데, 저장소가 공개라 누구나 같은 키로 서명할 수 있어 휴대폰(삼성 자동 차단 ·
  Play 프로텍트)이 "잠재적으로 유해한 앱" 으로 표시했다(2026-09-25 사용자 보고). 키는 사용자 PC 에서
  `android/make-release-key.bat` 이 한 번 만든다(`~/.olo/`, `~/.gradle/gradle.properties` 의 `olo.release.*`).
  클라우드 세션은 환경 변수 `OLO_RELEASE_KEYSTORE_B64` · `OLO_RELEASE_PASSWORD` 로 받는다. 키가 없으면 개발용
  키로 서명하고 파일 이름에 **`-devkey`** 를 붙인다 — 그 APK 는 사람에게 건네지 않는다. 개발용 키는 debug
  빌드(`OLO eBook (dev)`)에만 남았다. 키를 바꾸면 이미 깔린 앱을 덮어 설치할 수 없다(지웠다 깔면 책갈피 ·
  진도 · 형광펜이 사라진다) — 새 키는 절대 잃지 않게 따로 보관한다.
