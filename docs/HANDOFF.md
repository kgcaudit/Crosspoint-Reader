...
./gradlew :app:assembleRelease                # → app/build/outputs/apk/release/OLO-eBook-0.20.3-devkey-release.apk  (비공개 키가 있으면 -devkey 없음)
...
- 좋은생각 109쪽 글귀(SDGretaSans2, Adobe-KR 문자 모음): 번호가 Adobe-KR CID 였다. `BrokenHangul` 이 낱말마다 두 체계
  ...
  뽑아 `reader-pdf/src/main/resources/.../adobe-kr.txt`(번호 = 자리, 64KB)에 두었다 — **BSD 허가, 같은 자리의
  adobe-kr.LICENSE.txt 를 라이선스 화면(R4)에 꼭 넣는다.** 79쪽 "댦뗴꿹 뎚뎹" 으로 가려지던 줄도 "영토를 잃은 민족은…" 으로
