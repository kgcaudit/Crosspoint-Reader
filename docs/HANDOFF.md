...
```bash
cd android && ./gradlew check                 # 726개 + lint
./gradlew :app:assembleRelease                # → app/build/outputs/apk/release/OLO-eBook-0.12.0-release.apk
...
3 독서 기록 · 4 듣기). **1단계는 0.12.0 으로 끝났다.** 다음은 **두쪽보기**(사용자가 이번에 넣기로 함 — 구상안
다섯 장을 보였고 확정 대기), 그다음 2단계. 3단계(독서노트)는 구상안 N1–N9 결정 대기.
