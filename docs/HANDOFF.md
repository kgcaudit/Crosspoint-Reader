...
./gradlew :app:assembleRelease                # → app/build/outputs/apk/release/OLO-eBook-0.20.1-devkey-release.apk  (비공개 키가 있으면 -devkey 없음)
...
- 간헐 실패(원인 미확인, 이번 변경과 닿지 않음): 전체 점검에서 가끔 앱 시험 하나가 30초 기다리다 실패하고 따로 돌리면
