...
./gradlew :app:assembleRelease                # → app/build/outputs/apk/release/OLO-eBook-0.20.6-devkey-release.apk  (비공개 키가 있으면 -devkey 없음)
...
**0.20.6 — 폰 엔진은 줄바꿈 글자를 주지 않는다** (2026-09-26, 사용자 보고: 0.20.4 에서 좋은생각 109쪽 "있지 않았을까" 의 "있지" 뒤부터 깨짐)

- 근본 원인: 사용자가 0.20.5 의 진단 복사로 보내 준 폰 엔진의 글자 번호 · 위치를 작업 환경의 pdfium 과 맞대 보니, 번호와 위치는
  같고 **줄과 줄 사이의 "\r\n" 만 없었다.** `BrokenHangul` 은 줄바꿈 · 공백에서 낱말을 끊고 낱말마다 두 번호 체계 중 하나를
  고르는데, 줄바꿈이 없으니 본문 끝줄 "않았을까." (순번 체계 글꼴) + 쪽 번호 "107" + 글귀 첫 낱말 "번역은" (Adobe-KR 글꼴)이
  **한 낱말**이 되어 한 체계로 풀렸다 — 글귀 쪽이 이겨서 본문 "않았을까." 부터 깨졌다. 작업 환경의 pdfium 은 "\r\n" 을 넣어 주니
  재현되지 않았다.
- 두 번째 원인: 폰은 글귀 둘째 줄("안소니 버제스")을 **오른쪽에서 왼쪽 순서**로 준다. 0.20.3 의 `lines`(왼쪽으로 되돌아가면
  새 줄)로 나누면 한 글자씩 조각나, 한 글자 낱말은 두 체계 점수가 같아 순번 체계로 풀려 "닒뇽" 이 되었다.
- 고침: `PageText.rowStarts` — 글자 네모의 **높이가 절반 넘게 겹치지 않으면** 새 줄(왼쪽 되돌아감은 보지 않는다). `repaired()` 가
  이 줄 시작을 `BrokenHangul.repair(text, lineStarts)` 에 넘겨 낱말을 거기서도 끊는다. `inVisualOrder` 도 같은 줄로 나누어
  뒤집힌 줄을 바로 세운다. `PdfReader.textLayer` 순서: 고치기(줄 단위) → 화면 순서.
- 곁가지: 망가진 글꼴의 공백(번호 1) 사이에 홀로 선 기호("여정 @ 안소니")는 그 글꼴의 글자다 — 순번 체계로 '_'(+31).
  `loneMarks`. 이웃이 멀쩡한 글자면 건드리지 않는다(`user@mail.com`).
- 확인: 폰이 준 번호 그대로 넣으면 "여기에도 누군가의 숨은 노력이 있지 않았을까.107번역은 단순히 언어를 바꾸는 것이 아니라, 한
  문화를 다른 문화로 이동시키는 대담한 여정_ 안소니 버제스"(임시 탐침 시험, 저장소에 넣지 않음). 시험 `PageTextTest` 의 줄바꿈
  없는 층(순번 줄 + Adobe 줄, 뒤집힌 줄) · `BrokenHangulTest` 의 외톨이 기호. 돌연변이 4개(줄 자르기 · 줄 시작 안 넘김 ·
  외톨이 기호 · 왼쪽 되돌아감에서 끊기) 모두 잡힘. **실기기 확인 필요.**
- 진단 복사(`copyWithDiagnosis`)는 남겨 둔다 — 폰에서만 나는 글자 문제를 다시 만나면 같은 방법으로 받는다.

**0.20.5 — 진단: 복사에 엔진이 준 글자 번호** (2026-09-26)
