...
- `olo-test-collection.ttc` — 두 서체를 묶은 TTC. 0번은 `olo-test-regular` 에 **한국어 이름**
  ...
  (fontTools 로 만들었다.)
- `olo-test-variable.ttf` — 보통·굵게 두 서체를 마스터로 만든 **가변 폰트**(wght 400~700, "Olo Test Variable").
  두 서체의 점 구조가 다른 글리프는 굵기가 변하지 않지만 가나다 같은 글자는 변한다 — 가변 축을
  실제로 쓰는지 폭으로 확인하기에 충분하다. (fontTools varLib 로 만들었다.)
